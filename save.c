 ...
 * The two basic funciton here will be "save" and "restore".
 */
...
save_game()
{
    ...
    if (terse)
        ...
        printw("Save file (press enter (\x11\xd9) to default to \"%s\")? ", s_save);
 ...
 *      Allocatea enough data space so taht open data file information
 *      will be stored outside the data area that will be restored,
 *      Now reopen data save fie,
 *      skip header,
...
char *savefile;
{
    ...
    char errbuf[11], ssave_name[MAXSTR];
    chart *read_error = "Read Error";
...
rok:
    ...
    no_check = old_check;
    mpos = 0;
    ifterse1("%s, Welcome back!", "Hewllo %s, Welcome back to the Dungeons of Doom!", whoami);
