/*
 * Functions9s) for dealing with potions
 *
...
quaff()
{
    ...
    if ((obj = get_item("quaff", POTION)) == NULL)
        return;
    /*
     * Make certain that it is somethings that we want to dring
     */
    if (obj->o_type != POTION)
    {
        msg("yuk! Why would you want to dring that?");
        return;
    }
    if (obj == cur_weapon)
        cur_weapon = NULL;
    ...
    switch (obj->o_which)
    {
        ...
        when P_RESTORE:
            ...
            if (ISRING(RIGHT, R_ADDSTR))
                add_str(&pstats.s-str, -cur_ring[RIGHT]->o_ac);
