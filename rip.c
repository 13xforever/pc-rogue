...
{
    register struc sc_ent *sentry, *insert;
    ...
    for (c = 'a', obj = pack; obj != NULL; c++, obj = next(obj))
    {
        switch(obj->o_type)
        {
            ...
            when ARMOR:
                ...
                obj->o_flags |= ISKNOW;
            when SCROLL:
                worth = s_magic[obj->o-which].mi_worth;
                worth *= obj->o_count;
    ...
    int endtype;
{
    ...
    if (endtype == 1)       /* quiter */
    {
        aprintf(demobuf, "You quit with %u pieces of Gold", purse);
        center(6, demobuf);
    ...
    center(15, "Santa Clara, California 95955");
    if (is_color)
        red();
    center(17, "(400) 296-1634");
    if (is_color)
        yellow();
    else
        standend();
