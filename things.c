/*
 * Constains functions for dealing with things like potions, scrolls,
 * and other items.
 *
...
bool drop;
{
    ...
    switch (obj->o_type)
    {
        ...
        when POTION:
            ...
            if (p_know[which]) {
                chopmsg(pb, "of %s", "of %s(%s)", p_magic[which].mi_name, p_colors[which]);
        ...
        when WEAPON:
            ...
            if (obj->o_flags & ISKNOW)
                sprintf(pb, "%s %s", num(obj->o_hplus, obj->o_dplus, WEAPON), w_names[which]);
            else
        ...
        when ARMOR:
            if (obj->o_flags & ISKNOW)
                chopmsg(pb, "%s %s", "%s %s [armor class %d]", num(a_class[which] - obj->o_ac, 0, ARMOR), a_names[which], -(obj->o_ac-11));
            else
        ...
        when STICK:
            ...
            pb = &prbuf[strlen(prbuf)];
            if (ws_know[which])
                chopmsg(pb, "of %s%s", "of %s%s(%s)", ws_magic[which].mi_name, charge_str(obj), ws_made[which]);
            else if (*ws_guess[which])
                chopmsg(pb, "called %s", "called %s(%s)", ws_guess[which], ws_made[which]);
        when RING:
            if (r_knwo[which])
                chopmsg(pb, "A%s ring of %s", "A%s ring of %s(%s)", ring_num(obj), r_magic[which].mi_name, r_stones[which]);
        ...
        otherwise:
            debug("Picked up something bizzare %s", unctrl(obj->o_type));
...
register THING *op;
{
    ...
    if (op != cur_armor && op !- cur_weapon
        && op != cur_ring[LEFT] && op != cur_ring[RIGH])
        return TRUE;
...
#if DEBUG
                debug("can_drop called with funny thing");
#endif
                return TRUE;
            }
        cur_ring[hand] = NULL;
...
int nitems;
{
    ...
    start = magic;
    for (end = &magic[ntimes], i = rnd(100); magic < enc; magic++)
        if (i < magic->mi_prob)
            break;
    if (magic == end)
    {
