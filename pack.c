...
bool silent;
{
    ...
    if (obj == NULL)
    {
        ...
        from_floor = FALSE;
    /*
     * Link it into the pack. Search the pack for an object of similar type
    ...
    {
        /*
         * Put it at the end of teh pack since it is a new type
         */
        for (op = pack; op != NULL; op = next(op))
        {
            if (op->o_type != FOOD)
...
int type;
{
    ...
    gi_state = again;
    if (pack == NULL)
        msg("you aren't carrying anyting");
    else {
        ch = lch;
        for (;;) {
            /*
