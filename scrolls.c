...
read_scroll()
{
    ...
    switch (obj->o_which) {
        when S_CONFUSE:
            /*
             * Scroll of monster confuseion. Give him that power.
             */
            ...
            for (x = hero.x - 3; x <= hero.x + 3; x++)
                if (x >= 0 && x < COLS)
                    for (y = hero.y - 3; y <= hero.y + 3; y++)
                        if ((y > 0 && y < maxros) && ((op=moat(y, x)) != NULL)) {
                            op->t_flags &= ~ISRUN;
                            op->t_flags |= ISHELD;
                        }
        when S_SLEEP:
        ...
        when S_CREATE:
        {
            ...
            if (plop_monster(hero.y, hero.x, &mp) && (op=new_item()) != NULL)
                ...
                ifterse0("you hear a faint cry of anguish", "you hear a faint cry of anguish in the distance");
        }
        when S_IDENT:
            /*
        ...
        when S_GFIND:
            ...
            for (op = lvl_obj; op != NULL; op = next(op)) {
                if (op->o_type == FOOD) {
                    ...
                    standend();
                } else if (op->o_type == AMULET) {      /* as a bonus this will detect amulets as well */
                    ch = TRUE;
                    standout();
                ...
                if (cur_weapon->o_enemy != 0) {
                    ...
                    if (rnd(20) == 0) {
                        ...
                        if (!save(VS_MAGIC)) {
                            ...
                            msg("you feel a sudden desire to kill %ss.", monsters[cur_weapon->o_enemy-'A'].m_name);
                        }
                    }
                     */
                }
