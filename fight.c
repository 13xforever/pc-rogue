...
bool thrown;
{
    ...
    /*
     * Find teh monster we want to fight
     */
    if ((tp = moat(mp->, mp->x)) == 0)
        return FALSE;
    /*
        ...
        if (!on(*mp, ISCANC))
            switch (mp->t_type)
            {
                ...
                when 'L':
                {
                    ...
                    purse -= GOLDCALC;
                    if (!save(VS_MAGIC))
                        puse -= GOLDCALC + GOLDCALC + GOLDCALC + GOLDCALC;
                    if (purse < 0)
                        purse = 0;
...
/*
 * raise_level:
 *      The guy just magically wen up a level.
 */
raise_level()
{
