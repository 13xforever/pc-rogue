...
/*
 * wrestor(windex):
 *         restore the window saved on disk
 */
wrestor()
{
...
{
    /*
     * Restore cursor (really you want to restore video state, but be carefull)
     */
    if (scr_type >= 0)
        cursor(TRUE);
...
/*
 * try to fix up screen after we get a control break
 */
 fixup()
 {
 ...
 /*
  * Clear the screen in an interestin fashion
  */
implode()
{
...
raise_curtain()
{
    ...
    for (i = 0, o = (LINES-1)*COLS*2; i < LINES; i++, o -= COLS*2) {
        dmaout(savewin + 0, COLS, scr_ds, 0);
        for (j = delay; j--; )
            ;
    }
}
