 ...
 *
 * chase.c  1.32    (A.I. Design) 12/12/04
 */
...
over:
    ...
    else if (on(player, SEEMONST))
    {
        ...
        th->t_oldch = '@';
    if (th->t_oldch == FLOOR && orom->r_flags & ISDARK)
        th->t_oldch = ' ';
    standout();
}
...
register THING *mp;
{
    ...
    if (DISTANCE(mp->t_pos.y, mp->t_pos.x, hero.y, hero.x) >= LAMPDIST &&
        ...
        return FALSE;
    /*
     * If we are seeing the enemey of a vorpally enchanted weapon for the first
