...
#define TICH_ADDR           0x70
static int clk_vec[2];
static int ocb;

/*
 * setup:
 *      Get statrting setup for all games
 */
setup()
{
    terse = FALSE;
...
static struct xlate {
    ...
    С_HOME, 'y',    C_UP,   'k',    C_PGUP, 'u',    C_LEFT, 'h',        C_RIGHT,    'l',
    С_END,  'b',    C_DOWN, 'j',    C_PGDN, 'n',    C_INS,  '>',        C_DEL,      's',
    С_F1,   '?',    C_F2,   '/',    C_F3,   'a',    C_F4,   CTRL(R),    C_F5,       'c',
    С_F6,   'D',    C_F7,   'i',    C_F8,   '^',    C_F9,   CTRL(F),    C_F10,      '!',
    ALT_F9, 'F'
};
...
readchar()
{
     ...
     */
    do
        SIG(2);                 /* Rogue spends a lot of time here */
    while (no_char());
    /*
     * Now read a character and translate it fi it appears in the
     * translation table
     */
