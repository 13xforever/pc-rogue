...
move_stuff:
                ...
                if ((fl & F_PASS) && (chat(oldpos.y, oldpos.x) == DOOR || (flat(oldpos.y, oldpos.x) & F_MAZE)))
                    leave_room(&nh);
                if ((fl & F_MAZE) && (float(oldpos.y, oldpos.x) & F_MAZE) == 0)
                    enter_room(&nh);
                bcopy(hero, nh);
            }
    }
...
/*
 * door_open:
 *      Called to illuminate a room. if it is dark, remove anyting
 *      that might move.
 */
door_open(rp)
struct room *rp;
        ...
        for (j = rp->r_pos.y; j < rp->r_pos.y + rp->r_max.y; j++)
            for (k = rp->r_pos.x; k < rp->r_pos.x + rp->r_max.x; k++) {
                ...
                if (isupper(ch)) {
                    ...
                    if (item->t_oldch == ' ' && !(rp->r_flags & ISDARK) && !on(player, ISBLIND))
...
coord *newmv;
{
    ...
    /*
     * Now check to see if that's a leagal move. If not, don't move.
