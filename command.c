...
execcom()
{
    ...
    do {
        switch (ch = get_prefix()) {
            when 'h': when 'j': when 'k': when 'l':
            when 'y': when 'u': when 'b': when 'n':
                find_dir(ch, &mv);
                do_move(mv.y, mv.x);
            when 'H': when 'J': when 'K': when 'L':
            when 'Y': when 'U': when 'B': when 'N':
            ...
            when 'o': after = FALSE; msg("i don't have anyh options, oh my!");
            when CTRL(L):
                after = FALSE;
