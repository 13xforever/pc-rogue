...
; space first (so that we can avoid destroying the memeory variables)
; :ts=8
...
sbot    dw      0
_lowmem_:
        extern  _Uorg_:byte,_Uend_:byte
dataseg ends
...
noclear:
;
        mov     es,[2cH]    ;get environment segment
        sub     di,di
        mov     cx,7fffH
arglook:
...
mov_args:
        ...
        jcxz    cpy_done
cpy_args:                   ;copy argument string to weork buffer
        lodsb
        test    al,al
        jz      cpy_done
        stosb
